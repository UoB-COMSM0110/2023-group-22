...
void draw(){
  ...
  //pause window
    ...
    image(background_img, 0, 0);
    pushMatrix();
    if (doodler.get(0).velocity > 10 || doodler.get(0).updateMonster(monsters)) {
      noLoop();
      isGameOver = true;
      gameOver.draw();
    }
    ...
    if(setting.playerNumber != 0){
      ...
      doodler.get(1).springConstant = -10;
      doodler.get(1).maxSpeed = -12;
      doodler.get(0).springConstant = -8;
      doodler.get(0).maxSpeed = -10;
    }
